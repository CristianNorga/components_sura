  ...
  */
		...
		const unmount = (
			...
			if (shapeFlag & 6 /* COMPONENT */) {
				...
				if (shapeFlag & 64 /* TELEPORT */) {
					...
					(type === Fragment &&
						patchFlag & (128 /* KEYED_FRAGMENT */ | 256) /* UNKEYED_FRAGMENT */) ||
		...
		if (props == null || isString(props)) {
			...
			if (prop.key.type === 4 /* SIMPLE_EXPRESSION */) {
				...
				alreadyExists = props.properties.some(
					(p) =>
						p.key.type === 4 /* SIMPLE_EXPRESSION */ && p.key.content === propKeyName
				);
			}
			if (!alreadyExists) {
						...
						if (key) {
							sibling.branches.forEach(({ userKey }) => {
								if (isSameKey(userKey, key)) {
									context.onError(
										createCompilerError(29 /* X_V_IF_SAME_KEY */, branch.userKey.loc)
									);
								}
							});
						}
					...
					if (isTemplate) {
						node.children.some((c) => {
							if (c.type === 1 /* ELEMENT */) {
								...
								if (key) {
									context.onError(
										createCompilerError(33 /* X_V_FOR_TEMPLATE_KEY_PLACEMENT */, key.loc)
									);
									return true;
								}
							}
					...
					(children.length === 1 &&
						(node.type === 0 /* ROOT */ ||
							(node.type === 1 /* ELEMENT */ &&
								...
								!node.props.find(
									(p) =>
										p.type === 7 /* DIRECTIVE */ && !context.directiveTransforms[p.name]
								) &&
	...
	const transformModel$1 = (dir, node, context) => {
		...
		baseResult.props = baseResult.props.filter(
			(p) =>
				!(
					p.key.type === 4 /* SIMPLE_EXPRESSION */ && p.key.content === 'modelValue'
				)
		);
		return baseResult;
	};
