(function (root, factory) {
		...
		load: {
			jQuery: function (cb) {
				...
				if (
					...
					window.location.search.includes('use_jquery_3_5_1')
				) {
					jqueryUrl = '//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js';
				}
				LibUtils.getItem(
			...
			FuelTracking: function () {
				return LibUtils.getAsyncItem(
					!window.Fuel || !window.Fuel.Tracking || !window.Fuel.TrackingContext,
					'fuel-tracking-script',
					getAppDomainUri() + '/CloudPages/utilities/t.js',
					true
				);
			},
			jQueryValidatorMessages: function () {
				...
				const lang =
					langs.indexOf(navigatorLanguage) >= 0 ? navigatorLanguage : 'en';
				return LibUtils.getAsyncItem(
					true,
					'jquery-validator-messages',
					getAppDomainUri() +
						'/CloudPages/lib/sc-validation-messages/' +
						lang +
						'.js',
					true
				);
			...
			jQueryValidator: function () {
				return LibUtils.getAsyncItem(
					...
					getAppDomainUri() + '/CloudPages/lib/jquery.validator.js',
					true
				);
			},
			BootstrapDatepicker: function () {
				...
				LibUtils.getAsyncItem(
					datepickerLoaded,
					'jquery-ui-script',
					'//ajax.googleapis.com/ajax/libs/jqueryui/1.8.22/jquery-ui.min.js'
				);
				LibUtils.getAsyncItem(
					datepickerLoaded,
					'fuelux-loader-script',
					'//www.fuelcdn.com/fuelux/2.3/loader.min.js'
				);

				return LibUtils.getAsyncItem(
					datepickerLoaded,
					'bootstrap-datepicker-script',
					getAppDomainUri() + '/CloudPages/lib/bootstrap-datepicker.js',
					true
				);
			},
		},
	...
	ScForm = function (options) {
		...
		function modalZIndexFix() {
			...
			self.$smartCapture.find('.fuelux > .modal').parent().appendTo('body');
		}
		...
		function initializeDatePickers() {
			self.$smartCapture.find('.activefield-datepicker').each(activateDatePicker);
		...
		function initializeTabIndexes() {
			...
			var totalElements = $scInstances
				.find(elementTypesToInclude)
				.filter(':visible').length;
			var elements = self.$smartCapture
				.find(elementTypesToInclude)
				.filter(':visible');
		...
		function injectPersonalizationValues() {
			self.$smartCapture.find('[data-value]').each(function () {
				var $this = $(this);
	...
	return {
		init: function (options) {
			...
			LibUtils.createLink(
				...
				getAppDomainUri() + '/CloudPages/css/datepicker.css'
			);
			LibUtils.createLink(
				'smartcapture-stylesheet',
				getAppDomainUri() + '/CloudPages/css/smartcapture-form.css'
			);
