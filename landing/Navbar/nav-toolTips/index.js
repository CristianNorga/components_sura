	...
	beforeMount: function () {
		...
		cities.forEach((element) => {
			...
			element.items.forEach((item) => {
				let content = this.getCodeResumeCombo(item.resumeCombo);
				content += this.getCodeDescriCombo(item.descriCombo);
				content += this.getCodeCost(item.cost);
				...
				let itemTooltip = `<div class="tooltip-item col-${item.xs} col-sm-${
					item.sm
				} col-md-${item.ms} col-lg-${item.lg} col-xl-${item.xl}" style="${
					item.disabled ? 'background: #e2e2e2;' : ''
				}">${content}</div>`;
