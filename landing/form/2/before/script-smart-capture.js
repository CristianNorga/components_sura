	...
	var LibUtils = {
		...
		load: {
			jQuery: function (cb) {
				...
				if (window.location.search && window.location.search.includes && window.location.search.includes('use_jquery_3_5_1')) {
					jqueryUrl = '//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js';
				}
				LibUtils.getItem((!($ || window.jQuery || window.$)), 'jquery-script', jqueryUrl, function () {
					if (!$) {
